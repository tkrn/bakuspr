...
namespace bakuspr
{
  public partial class frmMain : Form
  {
    ...
    private void loadCustomPaletteToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ofdPalette.Filter = "Custom Palette|*.bin";
      if (ofdPalette.ShowDialog(this) == DialogResult.OK)
      {        
        rb_bgr_3bpp.Enabled = true;
        rb_rgb_3bpp.Enabled = true;
        rb_bgr_4bpp.Enabled = true;
        rb_rgb_4bpp.Enabled = true;
        changeCustomPaletteMethod();
      }
    }

    private void changeCustomPaletteMethod ()
    {
      byte[] file = File.ReadAllBytes(ofdPalette.FileName);
      grpPalette.Text = "Palette - " + ofdPalette.SafeFileName;

      Color[] loadedPalette = new Color[256];

      if (rb_bgr_3bpp.Checked)
      {
        loadedPalette = ImageHandler.Load_bgr3bpp_Palette(file);
      }
      else if (rb_rgb_3bpp.Checked)
      {
        loadedPalette = ImageHandler.Load_rgb3bpp_Palette(file);
      }
      else if (rb_bgr_4bpp.Checked)
      {
        loadedPalette = ImageHandler.Load_bgr4bpp_Palette(file);
      }
      else if (rb_rgb_4bpp.Checked)
      {
        loadedPalette = ImageHandler.Load_rgb4bpp_Palette(file);
      }
    ...
    private void rb_rgb_3bpp_CheckedChanged(object sender, EventArgs e)
    {
      changeCustomPaletteMethod();
    }

    private void rb_bgr_3bpp_CheckedChanged(object sender, EventArgs e)
    {
      changeCustomPaletteMethod();
    }

    private void rb_rgb_4bpp_CheckedChanged(object sender, EventArgs e)
    {
      changeCustomPaletteMethod();
    }

    private void rb_bgr_4bpp_CheckedChanged(object sender, EventArgs e)
    {
      changeCustomPaletteMethod();
    }

    private void loadJASCPaleteeToolStripMenuItem_Click(object sender, EventArgs e)
    {
      ofdPalette.Filter = "JASC Palette|*.pal";
      if (ofdPalette.ShowDialog(this) == DialogResult.OK)
      {
        rb_bgr_3bpp.Enabled = false;
        rb_rgb_3bpp.Enabled = false;
        rb_bgr_4bpp.Enabled = false;
        rb_rgb_4bpp.Enabled = false;

        byte[] file = File.ReadAllBytes(ofdPalette.FileName);
        grpPalette.Text = "Palette - " + ofdPalette.SafeFileName;

        Color[] loadedPalette = new Color[256];
        loadedPalette = ImageHandler.Load_JASC_Palette(file);
